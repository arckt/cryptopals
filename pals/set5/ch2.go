package main

import(
	"fmt"
	"math/big"
	"crypt"
	"encoding/binary"
)

func main() {
	p := new(big.Int)
	p.SetString("", 16)
	g := new(big.Int)
	g.SetString("1",10)
	fmt.Printf("Value of p: %v\n", p)
	fmt.Printf("Value of g: %v\n", g)
	a := big.NewInt(int64(binary.LittleEndian.Uint64(crypt.RndStr(8))))
	b := big.NewInt(int64(binary.LittleEndian.Uint64(crypt.RndStr(8))))
	A := new(big.Int)
	B := new(big.Int)
	A.Exp(g, a, p)
	B.Exp(g, b, p)
	fmt.Printf("A->M: p: %v g: %v A: %v\n", p, g, A)
	fmt.Printf("M->B: p: %v g: %v p: %v\n", p, g, p)
	fmt.Printf("B->M: B: %v\n", B)
	fmt.Printf("M->A: p: %v\n", p)
	s1 := new(big.Int)
	s2 := new(big.Int)
	s1.Exp(B, a, p)
	s2.Exp(A, b, p)
	s1b := s1.Bytes()
	s2b := s2.Bytes()
	msg1 := "they are here"
	rand1 := crypt.RndStr(16)
	fmt.Println(s1b)
	msg11 := append(crypt.EncAesCBC(crypt.PadPKCS7([]byte(msg1),16), crypt.SHA1(s1b)[:16], rand1),rand1...)
	msg21 := append(crypt.EncAesCBC(crypt.PadPKCS7([]byte(msg1),16), crypt.SHA1(s2b)[:16], rand1),rand1...)
	empty := []byte{byte(1)}
	fmt.Printf("A->M: msg: %v\n", msg11)
	fmt.Printf("Decrypted: %v\n", string(crypt.DecAesCBC(msg11[:len(msg11)-16], crypt.SHA1(empty)[:16],
		msg11[len(msg11)-16:])))
	fmt.Printf("B->M: msg: %v\n", msg21)
}
